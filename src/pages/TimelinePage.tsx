      ...
      <section className="panel">
        <div className="table-wrap">
          <table className="workshop-table">
            <thead><tr><th>Queue</th><th>Order / Piece</th><th>Operation</th><th>Work Center</th><th>Start</th><th>Finish</th><th>Dependencies</th><th>Confidence</th></tr></thead>
            <tbody>
              {operationTimeline.map((item, index) => (
                <tr key={item.operationId} className={item.confidence === 'LOW' ? 'row-due-soon' : ''}>
                  ...
                  <td>{item.dependencyIds.length}</td><td>{item.confidence}</td>
                </tr>
              ))}
            </tbody>
          </table>
